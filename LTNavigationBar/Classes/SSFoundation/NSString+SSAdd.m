...
- (NSMutableAttributedString *)paintStringInRange:(NSRange)range withColor:(UIColor *)color {
    return [self paintStringInRange:range withColor:color changeFontInRange:NSMakeRange(0, 0) withFont:nil];
}

- (NSMutableAttributedString *)changeFontInRange:(NSRange)range withFont:(UIFont *)font {
    return [self paintStringInRange:NSMakeRange(0, 0) withColor:nil changeFontInRange:range withFont:font];
}

- (NSMutableAttributedString *)paintStringInRange:(NSRange)range1 withColor:(UIColor *)color changeFontInRange:(NSRange)range2 withFont:(UIFont *)font {
    NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:self];
    if (color) {
        [str addAttributes:@{NSForegroundColorAttributeName:color} range:range1];
    }
    if (font) {
        [str addAttributes:@{NSFontAttributeName:font} range:range2];
    }
    return str;
}
